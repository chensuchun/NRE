...
wget -c http://ftp.gnu.org/gnu/gcc/gcc-${GCCVER}/gcc-${GCCVER}.tar.xz
wget -c ftp://sourceware.org/pub/newlib/newlib-${NEWLVER}.tar.gz

GCC_ARCH=gcc-$GCCVER.tar.xz
...
if $BUILD_BINUTILS; then
    if [ $REBUILD -eq 1 ]; then
        ...
        cd $ARCH && patch -p0 < binutils-$BINVER.diff
    fi
    cd $BUILD/binutils
...
if $BUILD_GCC; then
    if [ $REBUILD -eq 1 ]; then
        cat $GCC_ARCH | tar -C $SRC -Jxf -
        mv $SRC/gcc-$GCCVER $SRC/gcc
        cd $ARCH && patch -p0 < gcc-$GCCVER.diff
    fi
    cd $BUILD/gcc
    if [ $REBUILD -eq 1 ] || [ ! -f $BUILD/gcc/Makefile ]; then
        ...
        CFLAGS_FOR_TARGET="-g -O2 -D_POSIX_THREADS -D_UNIX98_THREAD_MUTEX_ATTRIBUTES -DPTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP" \
